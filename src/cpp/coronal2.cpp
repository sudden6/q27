...
private:
  static uint64_t countCompletions(uint64_t bv, uint64_t bh, uint64_t bu,
                                   uint64_t bd) {
    ...
    for (uint64_t slots = ~(bh | bu | bd); slots != 0;) {
      ...
      cnt +=
          countCompletions(bv, bh | slot, (bu | slot) << 1, (bd | slot) >> 1);
      slots ^= slot;
    }
    return cnt;
...
int main(int const argc, char const *const argv[]) {
  ...
  std::cout << "First side bound: (" << (unsigned)pres[(N / 2) * (N - 3)].a
            << ", " << (unsigned)pres[(N / 2) * (N - 3)].b << ") / ("
            << (unsigned)pres[(N / 2) * (N - 3) + 1].a << ", "
